...
interface DynamicLectureAttendanceProps {
  ...
  onRemoveAttendance: (subjectId: string, lectureNumber: number, date: string) => void;
}
...
export function DynamicLectureAttendance({
  ...
  const maxExistingLecture = existingRecords.length > 0 
  ...
  const getPresentCount = (): number => {
    ...
    return existingRecords
      .filter(r => r.lectureNumber <= lecturesForDay && r.present)
      .length;
  };
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-600">
            {lecturesForDay === 0 ? "No lectures (cancelled)" 
             : lecturesForDay === 1 ? "Single lecture" 
             : `${lecturesForDay} lectures today`}
          </p>
        </div>
        <div className="text-right">
          <div 
            className={cn("text-lg font-bold", getAttendanceColor(presentCount, lecturesForDay))}
            data-testid={`text-attendance-${subjectId}`}
          >
            {lecturesForDay === 0 ? "Cancelled" : `${presentCount}/${lecturesForDay}`}
          </div>
      ...
      <div className="flex items-center justify-center space-x-3 mb-4 p-3 bg-gray-50 rounded-lg">
        <span className="text-sm font-medium text-gray-700">Lectures today:</span>
        <div className="flex items-center space-x-2">
          ...
          <div 
            className="w-12 h-8 flex items-center justify-center text-lg font-medium bg-white border rounded"
      ...
      {lecturesForDay === 0 ? (
        <div className="text-center py-6 text-gray-500">
          ...
          <p className="text-xs mt-1">Set lectures to 1 or more to mark attendance</p>
        </div>
      ) : (
